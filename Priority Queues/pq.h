...
// Same class but giving priotity to larger number
...
public:
  int getSize() { return queue.size(); }
  bool isEmpty() { return queue.size() == 0; }
  int getMax() { return (isEmpty()) ? 0 : queue.at(0); }
  ...
  int removeMin() {
    ...
    while (childIndex1 < queue.size()) {
      ...
      if (queue[childIndex1] > queue[maxIndex])
        maxIndex = childIndex1;

      if (queue[childIndex2] > queue[maxIndex] && childIndex2 < queue.size())
      ...
      // swap otherwise
      std::swap(queue[parentIndex], queue[maxIndex]);
